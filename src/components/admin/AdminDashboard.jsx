...
export default function AdminDashboard() {
  ...
  const fetchApprovalRequests = useCallback(async () => {
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message && error.message.includes('relation "approval_requests" does not exist')) {
          alert(
            'Database Error: The approval_requests table does not exist.\n\n' +
            'Please create the table in your Supabase SQL Editor first.'
          );
        } else {
          alert('Error fetching approval requests: ' + (error.message || 'Unknown error'));
        }
        setApprovalRequests([]);
      } else {
    ...
    return (
      <div className="container py-5">
        <div className="alert alert-danger text-center">
          ...
          <p>Current role: <span className="badge bg-secondary">{user.role}</span></p>
  ...
  const handleReview = async (requestId, status, notes = '') => {
    ...
    try {
      const { error } = await reviewApprovalRequest(requestId, status, notes, user.id);
      if (error) {
                ...
                return (
                  <div key={request.id} className="col-lg-6 mb-4">
                    <div className="card h-100 border-0 shadow-sm">
                        ...
                        <div>
                          <h6 className="mb-0">
                            <span className="badge bg-primary me-2">
                              {request.role_requested.replace('_', ' ').toUpperCase()}
                            </span>
                            {fullName}
                          </h6>
                        </div>
                        ...
                        <div className="mb-3">
                          ...
                          <div className="row">
                            <div className="col-sm-6">
                              ...
                              <p className="mb-1">{userData.email || 'Not available'}</p>
                            </div>
                            <div className="col-sm-6">
                              <small className="text-muted">Phone:</small>
                              <p className="mb-1">{userData.phone_number || 'Not provided'}</p>
                            </div>
                          </div>
                        </div>

                      {/* Business Information */}
                      {request.role_requested === 'seller' && request.sellers && (
                        <div className="mb-3">
                          <h6 className="text-muted mb-2">Shop Information</h6>
                          <p><strong>Shop Name:</strong> {request.sellers.shop_name}</p>
                          <p><strong>Address:</strong> {request.sellers.shop_address}</p>
                          <p><strong>ID Card #:</strong> {request.sellers.id_card_number}</p>
                          {request.sellers.latitude && request.sellers.longitude && (
                            <p>
                              ...
                              >
                                View on Map <i className="bi bi-box-arrow-up-right"></i>
                              </a>
                            </p>
                          )}
                        </div>
                      ...
                      {request.role_requested === 'service_provider' && request.service_providers && (
                        <div className="mb-3">
                          <h6 className="text-muted mb-2">Service Information</h6>
                          <p><strong>Address:</strong> {request.service_providers.service_address}</p>
                          {request.service_providers.specializations && (
                            <div>
                              <strong>Specializations:</strong>
                              <div className="mt-1">
                                {Array.isArray(request.service_providers.specializations) ? 
                                  request.service_providers.specializations.map((spec, index) => (
                          ...
                          {request.service_providers.latitude && request.service_providers.longitude && (
                            <p>
                              <strong>Location:</strong>{' '}
                              <a 
