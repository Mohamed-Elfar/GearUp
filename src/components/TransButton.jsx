...
const TransButton = ({ className = "", mobileStyle = false }) => {
  ...
  return (
    <div className={`position-relative trans-button ${className}`}>
      <button
        className="btn d-flex align-items-center gap-2 text-white trans-main-btn w-100"
      ...
      {isDropdownOpen && (
        <div
          ...
          } shadow-lg rounded-3 bg-white border-0`}
          style={{
            padding: "0.5rem",
            marginTop: "0.25rem",
            minWidth: "120px",
        ...
        >
          {languages.map(({ code, label }) => (
            <button
              key={code}
              className={`btn text-start w-100 text-decoration-none py-2 px-3 ${
                code === language
